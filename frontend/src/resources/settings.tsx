...
/* The one section that is a list rather than a list of keys. It is a resource
 * of its own, drawn here as a resource is drawn — the package's table, its own
 * dialogs — under the sentence that says what the mapping is for. */
function ProjectsPage() {
  const section = sectionOf("projects")
  return (
    <div className="min-w-0">
      {section ? <Blurb text={section.blurb} /> : null}
      <ViewResourceContextProvider resource={projectPaths} resourceAction={ActionList.list} />
    </div>
  )
}
...
function pageFor(sectionKey: string): React.FC {
  const known = PAGES.get(sectionKey)
  if (known) return known
  const page = () => <SectionPage sectionKey={sectionKey} />
  PAGES.set(sectionKey, page)
  return page
}
...
function tabs(): SubViewResourceInterface[] {
  return (currentSettings()?.sections ?? []).map((section) => ({
    ...
    viewComponent: section.pairs === "projects" ? ProjectsPage : pageFor(section.key),
  }))
}
